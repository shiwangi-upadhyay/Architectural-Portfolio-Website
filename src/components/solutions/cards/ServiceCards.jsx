...
const services = [
  {
    ...
    icon: "/icons/architectural-engineering.svg",
  },
  {
    title: "3D Laser Scanning",
    icon: "/icons/3d-laser-scanning.svg",
  },
  {
    title: "BIM/VDC Services",
    icon: "/icons/bim-vdc-services.svg",
  },
  {
    title: "CAD Drafting",
    icon: "/icons/cad-drafting.svg",
  },
  {
    title: "Permit Facilitation",
    icon: "/icons/permit-facilitation.svg",
  },
];
    ...
    <div className="max-w-5xl mx-auto px-4 border-b-[3px] border-b-gray-200 pb-6">
      ...
      <div className="grid grid-cols-5 justify-center gap-6 mb-8">
        {services.map((service) => (
          <div
            key={service.title}
            className="bg-gradient-to-r from-orange-400 to-blue-900 rounded-lg shadow-lg w-46 h-30 flex flex-col items-center justify-center text-white hover:scale-105 transition"
          >
            <Image src={service.icon} alt={service.title} width={48} height={48} />
            <div className="mt-4 text-lg font-semibold text-center">{service.title}</div>
          </div>
        ))}
      </div>
       {/* Portfolio Button */}
